...
import './AtomForm.css';
import {fetchElectronSuccess, electronsFetch, fetchElectronRequest, fetchElectronFailure, fetchElectron} from '../actions/electron';
    ...
    <>
      <Form>
        <><Form.Field>
          ...
          {!dimension && 
          <MyDropdown value={valueM} setValue={setValueM} options={N} title={'Select M:'} />}
        </>
        <Container>
            <Button disabled={ !isDisabled } onClick={handleClick}>Start Simulation</Button>
        </Container>
      </Form>
  </>
  );
